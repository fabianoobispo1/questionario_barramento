...
export default function QuestionarioApp({ arquivoBase }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/${arquivoBase}.json`)
      ...
      .catch(() => setErroCarregamento('Não foi possível carregar este questionário.'));
  }, [arquivoBase]);
  ...
  const validarSecao = (secao: Secao): Erros => {
    ...
    for (const pergunta of secao.perguntas) {
      ...
      const vazio = !val || (Array.isArray(val) ? val.length === 0 : val.trim() === '');
      if (vazio) {
  ...
  const gerarMarkdown = (): string => {
    ...
    lines.push('- [ ] Lista de tabelas com volumetria diária em **Bytes**');
  ...
  const exportarMd = () => {
    ...
    a.href = url;
    a.download = `onboarding-${arquivoBase}.md`;
    a.click();
  ...
  const exportar = async (formato: 'tsv' | 'xlsx') => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `onboarding-${arquivoBase}.${formato}`;
      a.click();
      URL.revokeObjectURL(url);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
        <div className="max-w-lg w-full rounded-2xl border border-red-200 bg-white p-6">
          <h1 className="text-lg font-semibold text-red-700">Erro ao carregar</h1>
          <p className="text-slate-700 mt-2">{erroCarregamento}</p>
          <Link href="/" className="inline-flex mt-4 text-sm text-en-primary hover:underline">
            Voltar para seleção de questionários
          </Link>
        </div>
      </div>
    );
      ...
      <header className="bg-white border-b border-slate-200 shadow-sm sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-base font-bold text-slate-800 leading-tight">{questionario.titulo}</h1>
              <p className="text-xs text-slate-500">Seção {secaoAtiva + 1} de {totalSecoes}</p>
            </div>
          </div>
          <Link href="/" className="text-xs text-en-primary hover:underline">
            Trocar questionário
          </Link>
        </div>
        <div className="h-1 bg-slate-100">
          <div className="h-1 bg-en-secondary transition-all duration-500" style={{ width: `${progresso}%` }} />
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="bg-linear-to-r from-en-primary to-en-primary-hover px-6 py-4">
            ...
            {secao.descricao && <p className="text-en-primary-light text-sm mt-1">{secao.descricao}</p>}
          </div>
            ...
            {secao.perguntas.map((pergunta) => (
              <div key={pergunta.id}>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">
                  {pergunta.label}
                  {pergunta.obrigatorio && <span className="ml-1 text-red-500">*</span>}
                </label>
                <CampoFormulario
                  pergunta={pergunta}
                  respostas={respostas}
